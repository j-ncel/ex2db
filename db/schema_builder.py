...
class SchemaBuilder:
    ...
    def build_table_schema(self, df: pd.DataFrame, table_name: str) -> Table:
        metadata = MetaData()
        columns = []
    ...
    def summarize_sqlalchemy_types(self, df: pd.DataFrame) -> dict:
        return {
            col: self.map_dtype_to_sqlalchemy(dtype).__name__
